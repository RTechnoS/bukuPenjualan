...
class Dashboard:
	...
	def changeStore(self, *args):
		#print(self.__combo_store.current())
...
################### Bagian window utama ###########################
	...
	def def_frm_product(self):
		...
		tk.Label(frm_option, text = 'Jenis').grid(row=0, column=0, sticky="W")
		...
		tk.Label(frm_option, text = 'Product').grid(row=1, column=0, sticky="W")
		...
		tk.Label(frm_option, text = 'Pembayaran').grid(row=2, column=0, sticky="W")
		...
		tk.Label(frm_option, text = 'Harga').grid(row=3, column=0, sticky="W")
		self.__txt_price = tk.Label(frm_option, textvariable=self.var_price, width=27)
		self.__txt_price.grid(row=3, column=1)
		...
		tk.Button(frm_option, text='Submit', bg='#0086E6', command=self.sendSubmit).grid(row=4, column=0, sticky="W")
						...
						if int(self.var_amount.get()) > 0:
							...
							if int(self.harga) > 0:
								...
								if len(self.var_tanggal.get()) == 10:
									...
									FUNGSI.submitHistory(data['id_store'], data['id_product_type'],data['id_product'], data['id_via'], data['price'], data['time'], data['amount'], data['seller'])

		#print(data)
		#print(self.raw_store[self.__combo_store.current()][0])
		#data['store'] = 
